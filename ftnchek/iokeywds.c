/* 	$Id: iokeywds.c,v 1.7 2004/12/29 19:27:25 moniot Rel $
	  ...
	  if(k == IOKW_ACCESS) {
	    if( strcasecmp(modespec,"SEQUENTIAL") == 0 )
	      ...
	      current_io_access = IO_ACCESS_DIRECT;
				/* Some compilers accept ACCESS=APPEND */
	    else if( strcasecmp(modespec,"APPEND") == 0 ) {
	      if( f77_io_keywords || f90_io_keywords ) {
		   nonstandard(value->line_num,value->col_num,
			       f90_io_keywords,0);
		   if(f90_io_keywords) {
			msg_tail(": suggest use POSITION='APPEND' instead");
		   }
	      }
				/* treat as ACCESS=EQUENTIAL */
	      current_io_access = IO_ACCESS_SEQUENTIAL;
	    }
	    else {
	      syntax_error(value->line_num,value->col_num,
